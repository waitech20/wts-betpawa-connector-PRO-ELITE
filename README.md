...
## Phase 3 — Live Game Control

Phase 3 extends the authenticated connector with a full-screen live control dashboard.

### Aviator monitoring
- Detects the real Aviator canvas and game structure.
- Reads live multiplier text only when it is exposed through readable DOM content.
- Reads the connected BetPawa balance from `._balance_6umpy_40` / `.balance-amount`.
- Detects the two `.bet-control` blocks and their current amount/button readiness.
- Default displayed bet amount is `1.00 TZS` when the game reports that default.

### Manual bet controls
The dashboard exposes two controls labelled **PLACE BET — 2** and **PLACE BET — 3**.
They are strictly manual. No observer, threshold, timer, prediction, or background process clicks them automatically.
A click sends one authenticated manual-click request to the local connector, which verifies the live session and then clicks the corresponding real game Bet button.

The connector never chooses a bet for the user and never generates a crash prediction.
